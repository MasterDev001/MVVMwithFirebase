...
import com.example.mvvmwithfirebase.util.FirebaseStorageConstants.NOTE_IMAGES
...
class NoteRepositoryImpl(
    private val database: FirebaseFirestore, private val storageReference: StorageReference
) : NoteRepository {
    ...
    override fun deleteNote(note: Note, result: (UiState<String>) -> Unit) {
        ...
        database.collection(FireStoreCollection.NOTE).document(note.id).delete()
    ...
    override suspend fun uploadMultipleFile(
        fileUri: List<Uri>, onResult: (UiState<List<Uri>>) -> Unit
    ) {
        try {
